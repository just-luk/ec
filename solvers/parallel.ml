...
let parallel_do nc actions =
  ...
  while !finished_actions < number_of_actions do
    ...
    children := List.filter !children ~f:(fun p' -> not Core__.Import.Pid.(p = p'));
    (* Printf.printf "DEATH\n";
     *   flush_everything(); *)
...
let pmap ?processes:(processes=4) ?bsize:(bsize=0) f input output =
  ...
  while !total_computed < Array.length output do
    ...
    List.iter ~f:(fun descr ->
        ...
        let pid = List.Assoc.find_exn ~equal:(Core.Unix.File_descr.equal) !in_streams descr
      ...
      recvs.read;
    in_streams := List.filter ~f:(fun (stream,_) -> not (List.mem ~equal:(Core.Unix.File_descr.equal) recvs.read stream)) !in_streams;
  done;
  output
